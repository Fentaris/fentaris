...
Declare `auth: oauth()` on the server when the upstream runs an OAuth 2.1 flow. Fentaris then discovers the authorization server, registers a client, runs PKCE, refreshes tokens, and keeps one authorization per user.
...
<Note>
  A caller-supplied access token still works: pass it through a resolver and Fentaris forwards it unchanged. Use `oauth()` when nobody holds a token yet. See [Upstream OAuth 2.1](/guides/oauth).
</Note>
