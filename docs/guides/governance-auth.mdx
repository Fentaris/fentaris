...
The setup plan creates the local encryption key when necessary, asks invisibly for missing upstream tokens, and generates each declared Fentaris user API key. Save each generated API key when the CLI prints it. Fentaris stores only its hash. Clients send the raw value in the `x-fentaris-api-key` header.
