...
Use server handles when validation only applies to one upstream MCP server.

```ts theme={null}
proxy.mcp("analytics").use(async (ctx, next) => {
...
Prefer server-scoped middleware over global middleware when the rule only applies to one domain.
