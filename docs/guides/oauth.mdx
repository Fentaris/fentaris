...
The proxy and the CLI share the file without IPC: a CLI login is visible to a running proxy on its next read, with no restart. A headless or stdio exposure can refresh that stored authorization without declaring `redirectUrl`; a callback URL is only required to start or complete a new authorization-code flow.
