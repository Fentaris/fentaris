...
Use the high-level `rateLimit` helper for the common "N calls per window" case.

```ts theme={null}
import { rateLimit } from "@fentaris/core";

proxy.use(rateLimit({ max: 30, per: 60_000 }));
```

For production, pass a shared store to coordinate limits across multiple instances.
