...
When users declare API keys, Fentaris uses API-key identity by default with the `x-fentaris-api-key` header.
...
proxy.use(async (ctx, next) => {
  ...
  ctx.log.setTag("tenant", String(ctx.subject?.tenantId ?? "unknown"));
  return next();
});
```
