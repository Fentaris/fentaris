...
By default, Fentaris validates that policy server names are visible in the initial `fentaris(...)` config. If you declare policies first and attach upstream MCP servers with `app.mcp(...)` afterward, opt in to deferred policy server validation.
...
<Note>
  Keep the default validation unless your application intentionally registers upstream MCP servers after `fentaris(...)`. The deferred mode only skips the startup visibility error for policy references; requests still require the server to be registered before use.
</Note>
