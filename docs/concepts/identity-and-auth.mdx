...
API keys identify the caller to Fentaris. Upstream OAuth is the other direction: how Fentaris authenticates to a protected MCP server on that caller's behalf.

```ts theme={null}
mcp("linear", { transport: streamableHttp({ url: "https://mcp.linear.app/mcp" }), auth: oauth() })
```

Each Fentaris user gets their own upstream authorization, stored encrypted in `.fentaris/oauth-tokens.enc.json` and refreshed automatically. Sign a user in with `fentaris auth login linear --as user:alice`, or let the first tool call prompt them. See [Upstream OAuth 2.1](/guides/oauth).
