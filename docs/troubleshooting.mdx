...
### Edge deployment reports `install-required`

The deployment declares a managed install that is not ready yet. Read the reason
category from `fentaris edge status`: `install-denied` means the package is not
in `FENTARIS_EDGE_ALLOWED_PACKAGES`, `install-failed` means the package manager
exited non-zero or timed out, and `install-verification-failed` means the
installed version, the declared bin, or the declared integrity digest did not
match. Fentaris never falls back to a `PATH` executable for a managed install.
