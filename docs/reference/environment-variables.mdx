...
Local `fentaris secrets` and `fentaris auth api-key` commands resolve the key in this order: `--key`, the exported process environment, then the discovered project `.env`. A shell export overrides the project `.env`. When a local credential store already exists and no key is configured, interactive commands prompt for the encryption key.
