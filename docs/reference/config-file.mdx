...
<Note>
  Generated projects include `.gitignore` entries for `.fentaris/` because it contains local credentials, upstream OAuth tokens, and build output. The committed `.fentaris/secrets.manifest.json` schema is not ignored.
</Note>
...
```ts theme={null}
fentaris({
  servers: [mcp("linear", { transport: streamableHttp({ url }), auth: oauth() })],
  oauth: { publicUrl: "https://proxy.example.com", consentTimeoutMs: 90_000 },
});
```
...
<Note>
  See [Upstream OAuth 2.1](/guides/oauth) for the declaration forms and the consent flow.
</Note>
