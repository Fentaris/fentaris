...
Edge Control is also application configuration rather than a `fentaris.json` field. Set `McpProxyOptions.edge.control.enabled: true`, provide inventory and selection services, configure hard orchestration limits, and allow only the required `edge` tools in policy. See [Edge API Reference](/reference/edge#edge-control-configuration).
