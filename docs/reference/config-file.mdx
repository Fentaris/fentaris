...
<Note>
  Generated projects ignore `.fentaris/` in git because it contains local credentials and build output. The committed `.fentaris/secrets.manifest.json` schema is not ignored.
</Note>
...
Optional Panther Cloud project metadata for forward-compatible cloud sync. Cloud sync is not available yet; keep using the local encrypted store.
