...
### `validation.allowRuntimePolicyServerReferences`

Allows policies to reference upstream MCP server names that will be registered later through `proxy.mcp(...)`.

```ts theme={null}
const proxy = fentaris({
  validation: {
    allowRuntimePolicyServerReferences: true,
  },
  groups: [
    group({
      id: "limited",
...
Keep this disabled when all upstream MCP servers are known in `servers` or group `servers` at construction time.
