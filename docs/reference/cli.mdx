...
The command reads `FENTARIS_AUTH_KEY` from the exported environment or the discovered project `.env` before prompting. An exported value takes precedence over `.env`.
