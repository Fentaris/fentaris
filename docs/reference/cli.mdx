...
`auth list` returns each configured MCP key, its default selector, allowed selectors, and non-sensitive status. For a server declared with `oauth()`, `auth status` reports the stored authorization state and `auth login` runs the same browser login as `fentaris auth login`. Otherwise `auth login` returns machine-readable delegated login instructions.

### `fentaris auth login <mcp>`

Sign in to an upstream MCP server declared with `oauth()`. The command opens the authorization URL, receives the redirect on a loopback listener it owns, and writes the tokens to the project's encrypted OAuth store.
...
`--as` takes `user:<id>` or `shared`; omit it for the shared authorization. `--print-url` and `--non-interactive` never spawn a browser. `--port` fixes the loopback redirect port, which pre-registered clients need. A running proxy picks up the new tokens without a restart.
...
When `reference` or `--value` is omitted, the command walks through an interactive setup: pick a manifest reference, choose `default`, `user`, or `group` scope, enter the hidden value, review the redacted summary, and confirm before anything is written. Declining the confirmation leaves both `.env` and `.fentaris/credentials.enc.json` unchanged; a new project does not get an empty credential store.
