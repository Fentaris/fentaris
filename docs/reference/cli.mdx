...
`init` writes `fentaris.json`, `src/index.ts`, `.env.example`, local encrypted credentials, and package scripts.

### `fentaris dev`

Run the generated project's configured entrypoint.

```bash theme={null}
fentaris dev
```

The CLI discovers the nearest `fentaris.json` from the current directory upward, loads `.env` from the project root when it exists, prints the configured local endpoint, and runs `entrypoint` directly. TypeScript entrypoints run through the selected package manager's `tsx` executor.
