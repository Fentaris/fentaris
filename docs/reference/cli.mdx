...
```bash theme={null}
FENTARIS_EDGE_CONTROL_PLANE_URL="https://fentaris.example" \
FENTARIS_EDGE_ALLOWED_EXECUTABLES="filesystem-mcp" \
fentaris-edge login
```

The executable and package allowlists are deny-by-default. See
[Environment Variables](/reference/environment-variables) for configuration.
...
Revoke the enrolled device and delete its local identity, credentials, grants,
and setup secrets.
