...
The global, agent-specific install avoids adding skill files to the project before `fentaris init`. Then restart or reload your agent so the new skills are loaded.
...
* upstream OAuth 2.1 servers and current limits, including deploy support
...
Use `--all` only when every supported agent needs the skills. Prefer `-a <agent>` for a single agent; `-a '*'` is not a portable install or removal target.
