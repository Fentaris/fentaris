...
`fentaris()` creates the proxy application boundary. In this tutorial, the app constructor stays empty because project discovery, port, and path come from `fentaris.json`, and upstream MCP servers are attached with `app.mcp(...)`.
...
## Step 4: Attach a Diagnostics Server

Add a second upstream MCP server for internal diagnostics:
...
The `env` callback runs per request. It lets the upstream process receive request-scoped values without putting user-specific data into global process environment variables.

<Note>
  Replace `./dist/diagnostics-server.js` with a real MCP server entrypoint in your project. The tutorial focuses on the Fentaris wiring around multiple upstream MCP servers.
</Note>
...
```ts theme={null}
app.mcp("diagnostics").on("tool:success", async ({ ctx, durationMs }) => {
...
This hook only observes successful tool calls for the `diagnostics` upstream server. It does not affect filesystem calls and does not replace middleware or policy decisions.
...
* a server-scoped hook for diagnostics observability
