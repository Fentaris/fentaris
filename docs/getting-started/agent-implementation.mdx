...
`fentaris deploy` is not currently available; do not invent deploy commands.
Authenticate downstream clients with Fentaris API keys, trusted identity headers
behind an authenticated gateway, or an existing auth boundary. For an upstream
MCP server protected by OAuth 2.1, declare `auth: oauth()` and let Fentaris run
the flow; see [Upstream OAuth 2.1](/guides/oauth).
