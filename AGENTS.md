...
- It is a **nested pnpm workspace** with its own lockfile that consumes the *published* `@fentaris/*` packages (not the workspace source), so its checks work independently of the root build. It is excluded from the root `pnpm-workspace.yaml` (`!examples/team-governed-proxy`) so version PRs cannot desync the root lockfile before those versions exist on npm. That exclusion is also what keeps it out of Changesets — do not add it to the Changesets `ignore` list, because Changesets validates that every `ignore` entry resolves to a package in the project and aborts `version`/`publish` when it does not.
