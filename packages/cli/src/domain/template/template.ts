...
const runtimeProfiler = profiler()
  ...
  .on("mcp.call.success", (event) => {
    console.log(\`profile -> \${event.server ?? "unknown"}:\${event.operation} completed in \${event.durationMs ?? 0}ms\`);
  });
...
- A runtime profiler with a simple MCP success event example
...
- \`src/index.ts\` configures the app groups, profiler, and upstream MCP servers.
