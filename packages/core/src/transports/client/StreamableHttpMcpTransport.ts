...
export class StreamableHttpMcpTransport implements FentarisTransport {
   ...
   * @pk
   */
  withAuthProvider(authProvider: OAuthClientProvider): StreamableHttpMcpTransport {
    return new StreamableHttpMcpTransport({ ...this.options, authProvider }, this.user);
  }
  ...
  private async connect(): Promise<Client> {
    ...
    const transport = new StreamableHTTPClientTransport(new URL(this.options.url), {
      fetch: guardedUpstreamFetch(this.options.network, this.options.fetch),
